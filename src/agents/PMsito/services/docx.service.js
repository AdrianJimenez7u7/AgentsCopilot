...
async function buildChartPng({ categories, series, chartType }) {
  ...
  if (type === 'pie' || type === 'doughnut') {
    ...
    configuration = {
      ...
      data: { labels: categories, datasets: [{ label: series[0]?.name ?? 'Datos', data: dataValues, backgroundColor: backgroundColor.slice(0, categories.length) }] },
      options: { responsive: false, plugins: { legend: { position: 'right' }, title: { display: false } } },
    };
  } else {
    configuration = {
      type: 'bar',
      data: { labels: categories, datasets: series.map((s, idx) => ({ label: s.name ?? `Serie ${idx+1}`, data: s.data ?? [] })) },
      options: { responsive: false, plugins: { legend: { display: true }, title: { display: false } }, scales: { y: { beginAtZero: true } } },
    };
  }
